	...
	public class Config :IEnumerable
	{
		...
		public string this[string secname, string paramname]
		{
			get
			{
				...
				{
					//e.Message = "Ошибка в Config(индексатор)" + e.Message;
					try
					{
						l.WriteLine("Ошибка в Config(индексатор). Ошибка в методе Get(string secname, string paramname)\r\n\r\nsecname = " + secname + "\r\nparamname = " + paramname + e.Message, MessageType.Error);
					}
			...
			set
			{
				...
				{
					//e.Message = "Ошибка в Config(индексатор)" + e.Message;
					l.WriteLine("Ошибка в Config(индексатор). Ошибка в методе Set(string secname, string paramname)\r\n\r\nsecname = " + secname + "\r\nparamname = " + paramname + e.Message, MessageType.Error);
				}
			}
		}
		...
		{
			return Config.Parse(lhs.ToString()+rhs.ToString(),lhs.LoggerInstance);
		}
		...
		#endregion
		private static Config Parse(StringReader sr, Logger l)
		{
			Config destCol = new Config(l);
			string line;
			int number = 1;
		...
		{
			Config destCol = new Config(new Logger(null));
			string line;
			int number = 1;
			for (; (line = sr.ReadLine()) != null; number++)
			{
				...
				switch (line[0])
				{
					...
					default:
						...
						{
							destCol.LoggerInstance.WriteLine(e.Message, MessageType.Error);
							break;
						}
						break;
				}
		...
		public static Config Parse(string s,Logger l)
		{
			return Parse(new StringReader(s), l);
		}
		...
		*/
		...
		public static Config Load(string filename, Logger l)
		{
			StreamReader sr = null;
			string str = null;
			try
			{
				...
				{
					throw new IOException/*FileNotFoundException*/("Cannot open configfile: no such file:" + filename + "\n\n\n\n" + e.Message);
				}
				catch (UnauthorizedAccessException e)
				{
